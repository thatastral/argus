...
function actionLabel(action: ProposedAction): string {
  switch (action.type) {
    ...
    case "set_stake":
      return `Commit ${action.amount} ${action.assetSymbol} per habit`;
    case "withdraw":
...
export function ChatSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { setStake, busy: settingStake } = useSetStake();
  const toast = useToast();
  const actionBusy = creatingHabit || renamingHabit || deactivatingHabit || transferring || settingStake;
  ...
  async function resolveAction(messageIndex: number, actionIndex: number, confirm: boolean) {
    ...
    switch (action.type) {
      case "create_habit":
        ok = await createHabit(action.name);
      ...
      case "deposit":
        ...
        break;
      case "set_stake":
        ok = await setStake(action.amount, action.penaltyType, action.assetSymbol, action.assetDecimals);
        doneText = `Stake set to ${action.amount} ${action.assetSymbol} per habit.`;
        break;
      case "withdraw":
  ...
  return (
    ...
    >
      {/* TEMPORARY DEBUG PROBE — solid, unmistakable, no blur/mask/gradient complexity. If this
          doesn't show up either, the bug is structural (something preventing ANY absolutely-
          positioned child from painting visibly in this container), not a gradient/blur/mask
          issue in GlowBackground/DotGrid specifically. Remove once diagnosed. */}
      <div className="pointer-events-none absolute inset-0 z-0 bg-red-500 opacity-50" />

      <GlowBackground intensity={1.4} />
      <DotGrid intensity={1.4} />
